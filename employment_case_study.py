...
def get_data(acs_problem=ft.ACSEmployment, states=["GA"], years=['2014'], survey='person'):
    ...
    for year in years:
        ...
        # features, label, group = acs_problem.df_to_numpy(acs_data)
...
def plot_heatmaps(data: FolkAlgorithmData, clf, suptitle:str):
    ...
    # for val in data.data['RAC1P'].unique():
    #     data.data[data.data['RAC1P'] == val] = data.revert_value(val, 'RAC1P')
    #
    # data._data['RAC1P'] = data._data['RAC1P'].map(ORDINAL_MAP['RAC1P'])
    #
    # dummies = pd.get_dummies(data.data['RAC1P'], drop_first=False)
    # data._data = pd.concat([data.data, dummies], axis=1)
    # data._data.drop('RAC1P', axis=1, inplace=True)

    # x_cols = [col for col in data.data.columns if col not in data.label_names]
    # clf.fit(data.data.iloc[0:data._get_pivot()][x_cols], data.trainy)
    ...
    for key, group in data.test.groupby(data.group_name):
        ...
        stats[group_nm] = {
            ...
            "FDI": cms[group_nm].marginal_benefit().item(),
    ...
    for race1 in stats.columns:
        for race2 in stats.columns:
            ...
            fdi = stats[race1]["FDI"] - stats[race2]["FDI"]
            pp = stats[race1]["PP"] - stats[race2]["PP"]
            race_pairs[(race1, race2)] = {"TE": te, "DI": di, "FDI": fdi, "PP": pp}
    ...
    pivot_FDI = race_pairs.pivot(index="Race i", columns="Race j", values="FDI")
    ...
    sns.heatmap(pivot_FDI, annot=True, cmap=heat_color_map, ax=ax[3], fmt=".3f")
    ax[3].set_title('FDI Heatmap', fontsize=axis_title_size)
...
if __name__ == "__main__":
    ...
    data._train_frac = .6  # This is default, but just to help reviewers' understandability
    ...
    plt.savefig(cmn.RESULTS_DIR / "folktables_employment_fdi_nuance.png")
    # plot_heatmaps(data, clf=GaussianNB())
    # plt.savefig(cmn.RESULTS_DIR / "folktables_employment_fdi_nuance_NB.png")
